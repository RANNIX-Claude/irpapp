...
function generarHTML({ iniStr, finStr, pensiones, estac, vending, gastos, rentasEf, totales }) {
  ...
  const rowsGastos = gastos.map(g =>
    `<tr>
      ...
      <td style="padding:3px 6px;font-size:11px">${g.proveedor_nombre || '—'}</td>
      <td style="padding:3px 6px;font-size:11px">${g.descripcion || '—'}</td>
      <td style="padding:3px 6px;text-align:right;font-size:11px">${fmt(g.monto_pagado)}</td>
...
export default function ResumenSemanal() {
  ...
  const totGastosFondo= (datos?.gastos     ?? []).reduce((a, b) => a + (parseFloat(b.monto_pagado) || 0), 0)
                ...
                <tbody>
                  {gastos.length === 0 ? (
                    ...
                    <tr key={i} style={{ borderBottom: '1px solid #F3F4F6', background: i % 2 === 0 ? 'white' : '#FAFAFA' }}>
                      ...
                      <td style={{ padding: '6px 8px', fontWeight: 600, whiteSpace: 'nowrap', maxWidth: '100px', overflow: 'hidden', textOverflow: 'ellipsis' }}>{g.proveedor_nombre || '—'}</td>
                      <td style={{ padding: '6px 8px', color: '#374151', maxWidth: '120px' }}>{g.descripcion || '—'}</td>
                      <td style={{ padding: '6px 8px', textAlign: 'right', fontWeight: 700, fontFamily: 'monospace' }}>{fmt(g.monto_pagado)}</td>
