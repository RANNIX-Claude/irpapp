...
- [x] #014 `ingreso_id` en `subir-comprobante.js` se validaba con regex de UUID, pero `ingresos.id` es `bigint` — todo intento de guardar `comprobante_url` fallaba con "ingreso_id inválido" (la subida a Storage sí llegaba a completarse). Probablemente roto desde siempre, no es regresión de esta sesión. Corregido, probado en QA end-to-end (comprobante se ve completo en el modal) y desplegado a producción — 2026-09-18
