-- Reversión de seguridad — prod — foto tomada 2026-09-13T03:16:05.164Z
...
CREATE POLICY "comp_arrendatario" ON public.comprobantes_pago AS PERMISSIVE FOR ALL TO authenticated USING ((arrendatario_id = _comp_mi_arr_id())) WITH CHECK ((arrendatario_id = _comp_mi_arr_id()));
...
CREATE POLICY "arr_read_comp" ON storage.objects AS PERMISSIVE FOR SELECT TO authenticated USING (((bucket_id = 'comprobantes-pago'::text) AND (((storage.foldername(name))[1] = (_comp_mi_arr_id())::text) OR es_staff())));
CREATE POLICY "arr_upload_comp" ON storage.objects AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (((bucket_id = 'comprobantes-pago'::text) AND ((storage.foldername(name))[1] = (_comp_mi_arr_id())::text)));
